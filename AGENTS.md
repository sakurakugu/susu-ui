...
- 在 `packages/vue/src/index.ts` 中导入、导出并加入全量安装列表
- 在 `packages/vue/vite.config.ts` 中加入按需构建入口
- 在 `packages/vue/package.json` 的 `exports` 中加入子路径导出
- 在 `playground/src/playgroundNav.ts` 中加入导航
- 在 `playground/src/components/PlaygroundContent.vue` 中加入示例
- 如需示例样式，写入 `playground/src/style.css`
- 在 `docs/组件` 中新增组件文档
- 更新 `docs/README.md` 和根目录 `README.md` 的文档索引
